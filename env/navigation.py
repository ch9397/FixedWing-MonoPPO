...
class LocalNavigation:
    ...
    def distance_to_go(distance: float, off_tk_angle: float) -> float:
        ...
        """
        off_tk_angle_radians = math.radians(off_tk_angle)
        distance_to_go = distance * math.sin(off_tk_angle_radians)
        return distance_to_go

    @staticmethod
    def unit_dir_vector(start_point: tuple, end_point: tuple) -> tuple:         #计算两点之间单位向量
        """
        Calculate the unitary direction vector between 2 2D points
